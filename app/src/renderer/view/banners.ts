 ...
 * whose host emits `session:state` on every real change — of which a session
 * that reached a prompt has at least the boot latch, and the field is sticky
 * once set — so that function's `sessionState !== null` cue holds the view. Incidental rather
