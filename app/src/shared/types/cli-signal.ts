 ...
 *
 * INJECTION HISTORY. D2 U3 (probe `h4-model-switch-hooks`) injected
 * `PostModelSwitch` as the confirm for Sonata's mid-session model DRIVE, matched
 ...
 *    raise no dialog it lands 159ms and 243ms after the CR. The DIFFERENCE is
 *    stable where those absolutes are not: `Post` − stream receipt = 48–53ms
 *    across all six legs producing both.
 */
export type HookEventName =
  ...
  | "PreCompact"
  | "PostCompact"
  /** Claude only, MEASURED — the model switch pair. Neither is injected: the
   *  one consumer (Sonata's own mid-session model drive) was removed. */
  | "PreModelSwitch"
  | "PostModelSwitch"
