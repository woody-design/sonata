 ...
 *
 * Why this exists: our runtime settings files embed `<script>` commands that the
 * provider CLIs run via a SEPARATE, external interpreter process (Sonata-as-node
 * under `ELECTRON_RUN_AS_NODE`, or the `${SONATA_NODE:-node}` fallback PATH node).
 * That process has no Electron asar shim, so it cannot read anything inside
 * `app.asar` — a command pointing at `.../app.asar/dist/runtime/.../hook-sink.js`
 * fails with MODULE_NOT_FOUND. The scripts ARE unpacked to disk (electron-builder
 ...
 * to the external interpreter points at the unpacked file that exists on disk.
 *
 ...
 *
 * Cross-checked in a packaged build: the `app.asar` form is unreadable by the
 * external interpreter (MODULE_NOT_FOUND); the rewritten `app.asar.unpacked` form
 * runs (exit 0, payload written).
 */
