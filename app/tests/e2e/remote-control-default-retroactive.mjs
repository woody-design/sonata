...
const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "sonata-rcretro-userdata-"));
const selectedFolder = fs.mkdtempSync(path.join(os.tmpdir(), "sonata-rcretro-folder-"));
...
try {
  ...
  await sendFirstPrompt(page, "Reply with exactly: RETRO_SEED", { provider: "claude" });
