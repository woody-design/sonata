...
const success =
  ...
  // Neither model-switch hook is injected on any spawn shape. `PostModelSwitch`
  // was injected (D2 U3) only as the confirm for Sonata's mid-session model
  // drive; the Subtraction program removed the drive, and an injected event with
  // no reader is a line item nobody can retire (SL-9) — and not free, since the
  // CLI paints `Running <Event> hooks…` on the co-visible Terminal while it runs
  // them. `PreModelSwitch` was never injected (it fires on cancelled attempts too,
  // h4 arms b1/b2).
  [fastSettings, standardSettings, rcOffSettings, rcOnSettings].every(
    (settings) =>
      !("PostModelSwitch" in settings.hooks) && !("PreModelSwitch" in settings.hooks),
  ) &&
